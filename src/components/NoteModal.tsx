...
export const NoteModal: React.FC<NoteModalProps> = ({ probId, probName, onClose, onShowToast }) => {
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-fadeIn">
      <div
        className="w-full max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-2xl flex flex-col gap-4 transform transition-all scale-100"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 truncate max-w-[320px] font-semibold">{probName}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full h-56 p-3.5 rounded-xl bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700/80 text-xs font-mono text-zinc-900 dark:text-white outline-none focus:border-indigo-500 dark:focus:border-indigo-400 transition-colors resize-none"
            autoFocus
          />
        </div>
