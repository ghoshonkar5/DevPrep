...
import { ExternalLink, Star, FileText, SearchX } from 'lucide-react';
...
export const ProblemTable: React.FC<ProblemTableProps> = ({ onOpenNote, onShowToast }) => {
  const { activeTab, dsaOption, activePhaseId, filters, getStatus, setStatus, isBookmarked, toggleBookmark, getNote, resetFilters, isDoneInOtherList } = useStateContext();
  ...
  const renderContent = () => {
    ...
    if (matchCount === 0) {
      return (
        <div className="p-16 text-center bg-zinc-50 dark:bg-zinc-900 border border-dashed border-zinc-300 dark:border-zinc-800 rounded-2xl my-4">
    ...
    return (
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-xs bg-white dark:bg-zinc-900">
