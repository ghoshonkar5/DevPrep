...
export const Sidebar: React.FC = () => {
  const { activeTab, setActiveTab, dsaOption, setDsaOption, activePhaseId, setActivePhaseId, getStatus } = useStateContext();
  const roadmap = activeTab === 'dsa' ? (dsaOption === 'standard' ? INSTA_DSA_ROADMAP : DSA_ROADMAP) : activeTab === 'sql' ? SQL_DATA : CS_CORE_ROADMAP;

  return (
    <nav className="w-72 shrink-0 bg-zinc-50 dark:bg-zinc-900/90 border-r border-zinc-200 dark:border-zinc-800 flex flex-col z-10 transition-colors duration-200">
      {/* Header Tabs */}
      <div className="p-5 pb-3 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900">
        <div className="text-[11px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-3">
          Curriculum Roadmaps
        </div>
